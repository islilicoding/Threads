...
void *producer(void *ptr) {
  ...
  for (size_t i = 0; i < strlen(g_prod_str); i++) {
    /* Wait on the semaphore and obtain the lock, */
...
void *consumer(void *ptr) {
  ...
  for (size_t i = 0; i < strlen(g_prod_str); i++) {
    /* Wait on the semaphore and obtain the lock */
    /* then consume g_prod_str[i] from the g_buffer */
...
int main(int argc, char **argv) {
  ...
  /* Get number of producers and consumers from arguments */
  ...
  pthread_t * prod = (pthread_t *)malloc(sizeof(pthread_t) * prod_num);
  pthread_t * cons = (pthread_t *)malloc(sizeof(pthread_t) * cons_num);
  ...
  for (int i = 0; i < prod_num; i++) {
    ...
    *prod_id = i;
    pthread_create(&prod[i], NULL, (void * (*)(void *))
       producer, (void *)prod_id);
  }
  ...
  for (int i = 0; i < cons_num; i++) {
    ...
    *cons_id = i;
    pthread_create(&cons[i], NULL, (void * (*)(void *))
        consumer, (void *)cons_id);
  }

  /* Wait for the pruducer and consumer threads to finish */
